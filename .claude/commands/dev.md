...
Steps:
1. Check if `services/rag-service/.env` exists. If not, copy from `.env.example` and confirm `MOCK_EMBEDDINGS=true` and `MOCK_GENERATION=true` are set.
2. From `services/rag-service/`, run `uvicorn main:app --reload --port 8000`.
